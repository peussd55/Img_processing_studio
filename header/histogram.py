...
def calc_histo(image, rois, masks):
    ...
    subs = [image[y:y+h, x:x+w] for x, y, w, h in rois]             # 이미지에서 각부위 잘라냄
    hists = [cv2.calcHist([sub], [0,1,2], mask, bsize, ranges) for sub, mask in zip(subs, masks)]   #히스토그램 계산
    hists = [ h/np.sum(h) for h in hists]           # 정규화

    sim1 = cv2.compareHist(hists[2], hists[3], cv2.HISTCMP_CORREL)      #입술영역간 유사도
    sim2 = cv2.compareHist(hists[0], hists[1], cv2.HISTCMP_CORREL)      #머리카락영역간 유사도
    return  sim1, sim2